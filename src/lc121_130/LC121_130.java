...
class Solution122 {
	public int maxProfit(int[] prices) {
		...
		for (int i=1;i<prices.length;i++)
		{
			...
			{
				me[i]=Math.max(me[i],
						Math.max(0,prices[i]-prices[j])+me[j-1]);
			}
...
class Solution123_2
{
	public int maxProfit(int[] prices) {
		...
		{
			maxsince[i]=Math.max(maxsince[i+1],
					maxpri-prices[i]);
		...
		if (maxyet[pl-1]>maxa) maxa=maxyet[pl-1];
		return maxa;
	}
}
...
class Solution124 {
	public RST124 travel(TreeNode root)
	{
		...
		if (root.left!=null) LR=travel(root.left);
		if (root.right!=null) RR=travel(root.right);
		...
		if (LR!=null && RR!=null &&
				LR.LinkMaxPath+RR.LinkMaxPath+root.val>r.FinMaxPath)
...
class Solution125 
{
	public boolean isPalindrome(String s) 
	{
		...
		for (int i=0;i<n;i++)
			if (s.charAt(i)>='a' && s.charAt(i)<='z'||
			s.charAt(i)>='0' && s.charAt(i)<='9') str[m++]=s.charAt(i);
...
class Solution126 
{
	...
	private void establishAllWordList(String beginWord, 
			String endWord, List<String> wordList)
	{
		...
		for (int i=0;i<wordList.size();i++)
		{
			if (wordList.get(i).compareTo(beginWord)!=0
					&& wordList.get(i).compareTo(endWord)!=0)
	...
	private boolean BFS()
	{
		...
		for (int i=0;i<used.length;i++)
			used[i]=false;
		q[0]=0;prelen[0]=0;used[0]=true;dist[0]=0;
			...
			String endWord, List<String> wordList) 
	{
		...
		if (!EWInWL( endWord,  wordList))return ans;
	...
	public int longestConsecutive(int[] nums) {
		...
		while (i<nums.length)
		{
			...
			while (j<nums.length && nums[j]<=nums[j-1]+1)
				j++;
			if (nums[j-1]-nums[i]+1>maxl) maxl=nums[j-1]-nums[i]+1;
			i=j;
		}
		return maxl;
	}
...
class Solution130 
{
	...
	void bfs(char[][] board,int x,int y)
	{
		...
		while (h<r)
		{
			...
			h++;
			if (nx==0 || ny==0 || nx==l1-1 || ny==l2-1) onbd=true;
			for (int i=0;i<4;i++)
			{
				int mx=nx+di[i][0],my=ny+di[i][1];
				if (mx>=0 && my>=0 && mx<l1 && my<l2 && !checked[mx][my]
						&& board[mx][my]=='O')
				{
					checked[mx][my]=true;
					q[r][0]=mx;
	...
	public void solve(char[][] board) 
	{
		...
		for (int i1=0;i1<l1;i1++)
			for (int i2=0;i2<l2;i2++)
				if (board[i1][i2]=='O' && !checked[i1][i2]) bfs(board,i1,i2);
	}
}
