...
//290. Word Pattern
//Runtime: 1 ms, faster than 98.37% of Java online submissions for Word Pattern.
//Memory Usage: 35.4 MB, less than 36.58% of Java online submissions for Word Pattern.
class Solution290
{
	public boolean wordPattern(String pattern, String str)
	{
		...
		for (int i = 0; i < pt.length; i++)
		{
			...
			if (map[c] == null)
				map[c] = ws[i];
			else if (map[c].compareTo(ws[i]) != 0)
				return false;
		}
		for (int i = 0; i < 26; i++)
			for (int j = i + 1; j < 26; j++)
				if (map[i] != null && map[j] != null && map[i].compareTo(map[j]) == 0)
					return false;
		return true;
	}
}
