...
class Solution91_2 {
	...
	public int numDecodings(String s) {
		...
		d[0]=1;
		if (str[0]>='1' && str[0]<='9') d[1]=1;
		else d[1]=0;
		for (int i=1;i<str.length;i++)
		{
			if (str[i]>='1' && str[i]<='9') d[i+1]=d[i];
			else d[i+1]=0;
			if (str[i-1]=='1') d[i+1]+=d[i-1];
			if (str[i-1]=='2' && str[i]<='6') d[i+1]+=d[i-1];
		}
		return d[str.length];
	}
}
...
class Solution97 {
	public boolean isInterleave(String s1, String s2, String s3) {
		...
		for (int i=1;i<=s1.length();i++)
		{
			...
			for (int j=1;j<=s2.length();j++)
			{
				dp[i][j]=(s3.charAt(i+j-1)==s1.charAt(i-1)
						&& dp[i-1][j])||
...
class Solution97_2 {
	...
	{
		if (s3.length()!=s1.length()+s2.length()) return false;
		if (s1.length()==0) return s2.compareTo(s3)==0;
...
class R98
{
	boolean v;long min,max;
	R98(boolean v1,long min1,long max1)
	{v=v1;min=min1;max=max1;}
}
class Solution98 {
	R98 check(TreeNode root)
	{
		if (root==null) 
			return new R98(true,(long)(22)+Integer.MAX_VALUE,
					(long)(-22)+Integer.MIN_VALUE);
...
class Solution99 {
	public R99 check(TreeNode root)
	{
		if (root.left==null && root.right==null)
		{
			R99 R=new R99();
			R.ori=root;
			R.maxNode=root;R.minNode=root;
			R.legal=true;
			return R;
		}
		R99 RL=null,RR=null;
		if (root.left!=null) RL=check(root.left);
		if (root.right!=null) RR=check(root.right);			
	...
	{
		int t=r1.val;r1.val=r2.val;	r2.val=t;
	}
	public void recoverTree(TreeNode root) {
		...
		if (root.left!=null)
		{
			if (root.right!=null)
			{
				...
				else if (RL.maxNode.val>root.val && 
						RR.minNode.val>=root.val)
				{
				...
				{
					if (!RL.legal) recoverTree(root.left);
