...
class Solution22
{
	public static void dfs(int n,char[] st, int sp,
			int leftused,
			int sum,
			List<String> ans)
	{
...
//    	startTime=System.currentTimeMillis(); 
        ...
        {
        	for (int j=0;j<wn;j++) used[j]=false;
...
class Solution30_2 {
	...
	public List<Integer> merge(int l, int r, List<List<Integer>> lists) {
		...
		List<Integer> p1 = merge(l, (l + r) / 2, lists), 
				p2 = merge((l + r) / 2 + 1, r, lists);
		List<Integer> ans=new ArrayList<Integer>();
		int i1=0,i2=0,l1=p1.size(),l2=p2.size();
		while (i1<l1 || i2<l2 ) {
			if (i1==l1) 
			{
				while (i2<l2) ans.add(p2.get(i2++));
			} 
			else if (i2==l2) 
			{
				while (i1<l1) ans.add(p1.get(i1++));
			} 
			else 
			{
				if (p1.get(i1) < p2.get(i2)) ans.add(p1.get(i1++));
				else ans.add(p2.get(i2++));
			}
		}
		return ans;
	}
	...
	public List<Integer> FindMatch(List<Integer>S,int shift)
	{
		...
		{
			for (int j=0;j<Type.size();j++) ct[j]=0;
			int j=i;
			for (j=i;j<i+WN;j++)
			{
				if (S.get(j)<0) break;
				if (ct[S.get(j)] == Cap.get(S.get(j)))break;
				ct[S.get(j)]++;
			}
			if (j==i+WN) A.add(i*WL+shift);
		}
		return A;
	}
    public List<Integer> findSubstring(String s, String[] words) {
...
public class LC21_30 {
	...
	{
//		System.out.println(new Solution30_2().findSubstring(
//				"barfoothefoobarman",
//				new String[] {"foo","bar"}
//				));
		System.out.println(
				new Solution30_2().findSubstring(T30.s,T30.words));
	}
