...
class Solution101 {
	...
	void trav(TreeNode r,int l,int x)
	{
		...
		exi[x]=true;
		tarr[x]=r.val;
		if (r.left!=null) trav(r.left,l+1,2*x);
		if (r.right!=null) trav(r.right,l+1,2*x+1);
	}
    public boolean isSymmetric(TreeNode root) {
        ...
        for (int i=1;i<=maxL;i++)
        {
        	...
        	{
        		if (exi[start+j] && !exi[start*2-1-j]
        				||!exi[start+j] && exi[start*2-1-j])
        			return false;
        		if (exi[start+j] && tarr[start+j]!=
        				tarr[start*2-1-j])
        			return false;
        	}
        }
...
class Solution102 {
	...
	void Trav(TreeNode r,int layer)
	{
		...
		if (r.left!=null) Trav(r.left,layer+1);
		if (r.right!=null) Trav(r.right,layer+1);
	}
