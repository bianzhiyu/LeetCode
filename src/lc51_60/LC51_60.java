		...
		while (true)
		{
			...
			while (i < n)
			{
				...
				for (int j = 0; j < i; j++)
					...
					{
						intervals.set(j, mergeint(intervals.get(i), intervals.get(j)));
...
class Solution57
{
	public List<Interval> insert(List<Interval> intervals, Interval newInterval)
	{
		...
		int c1 = 0, c2 = 0, i = 0, j = 0, l = intervals.size(), c = newInterval.start, d = newInterval.end;

		if (c < intervals.get(0).start)
			c1 = 1;
		else if (c > intervals.get(l - 1).end)
			c1 = 2;
		else
		{
			int isContained = 0;
			...
			else
			{
				c1 = 4;
				for (i = 0; i < l - 1; i++)
					if (intervals.get(i + 1).start > c)
						break;
			}
		}

		if (d < intervals.get(0).start)
			c2 = 1;
		else if (d > intervals.get(l - 1).end)
			c2 = 2;
		else
		{
			int isContained = 0;
