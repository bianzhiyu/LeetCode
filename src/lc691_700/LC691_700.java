...
class P692 implements Comparable<P692>
{
	String wd;
	int fq;
...
class Solution698
{
	public boolean canPartitionKSubsets(int[] nums, int k)
	{
		...
		for (int i=0;i<k;i++)
		{
			...
			for (int j=0;j<nums.length;j++)
				if(used[j]) System.out.print(nums[j]+" ");
			System.out.println();
		}
		return true;
	}
...
class Solution698_2
{
    ...
    {
        if (k==0) return true;
        if (sum==0) return dfs(k-1,s,n.length-1);
        for (int i=maxRange;i>=0;i--)
        	if (!used[i] && sum>=n[i])
        	{
        		used[i]=true;
        		if (dfs(k,sum-n[i],i-1)) return true;
        		used[i]=false;
        	}
        return false;
    }
...
class Solution698_4
{
    ...
    {
        if (k==0) return true;
        if (sum==0) return dfs(k-1,s);
        for (int i=n.length-1;i>=0;i--)
        	if (!used[i] && sum>=n[i])
        	{
        		used[i]=true;
        		if (dfs(k,sum-n[i])) return true;
        		used[i]=false;
        	}
        return false;
    }
...
public class LC691_700
{
	...
	{
		int[] n=new int[] {10,10,10,7,7,7,7,7,7,6,6,6};
