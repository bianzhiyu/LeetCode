...
class WordDictionary
{
    ...
    private void pinsert(char[]str,int p)
	{
		...
		{
			if (son[str[p]]==null) son[str[p]]=new WordDictionary();
...
class Trie
{
	...
	{
		if (s.length==0 || strsp>=s.length) return;
		if (strsp==s.length-1)
		{
    ...
    public boolean containsNearbyDuplicate(int[] nums, int k) 
    {
    	...
    	for (int i:hm.keySet())
    		for (int j=0;j<hm.get(i).size()-1;j++)
    			if (hm.get(i).get(j+1)-hm.get(i).get(j)<=k) return true;
    	return false;
    }
}
    ...
    public boolean containsNearbyAlmostDuplicate(int[] nums, int k, int t) 
    {
        for (int i=0;i<nums.length;i++)
            for (int j=Math.max(0,i-k);
                 j<=Math.min(nums.length-1,i+k);j++)
                if (i!=j && 
                Math.abs((double)nums[i]-nums[j])<=(double)t+0.5
                    )
                    return true;
        return false;
