...
class Solution62_2 {
	...
	public int uniquePaths(int m, int n) {
		m--;n--;
		if (m>n) return (int) Comb(m+n,n);
		else return (int) Comb(m+n,m);
	}
}
//63. Unique Paths II
...
class Solution69 {
	...
	//l<=r, l^2<=x<=r^2
	{
		...
		if (r==l+1)
		{
			if (((long)r)*r==(long)x) return r;
			return l;
		}
		int m=(int)((long)l+r)/2;
		if (((long)m)*m>=(long)x) return sch(l,m,x);
		return sch(m,r,x);
	}
...
public class LC61_70 {
	...
	{
		System.out.println(new Solution63().uniquePathsWithObstacles(
				new int[][] {{1,0}}
				));
	}
