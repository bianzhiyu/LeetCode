	...
	public int[] kthSmallestPrimeFraction(int[] A, int K) {
		...
		for (int i=0;i<A.length;i++)
			for (int j=i+1;j<A.length;j++)
				if (!(i==0 && j==1)) tr=tr.insert(A[i], A[j]);
		for (int i=1;i<=K-1;i++)
			tr=tr.removeMin();
		tr=tr.getMinNode();
...
//Runtime: 8 ms, faster than 91.10% of Java online submissions for K-th Smallest Prime Fraction.
//Memory Usage: 40.1 MB, less than 48.00% of Java online submissions for K-th Smallest Prime Fraction.
class Solution786_2 {
...
public class LC781_790 {
	...
	{
		int[]a=new Solution786().kthSmallestPrimeFraction(
				new int[]{1, 2, 3, 5}, 
				3);
