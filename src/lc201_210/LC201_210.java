...
class Solution201 
{
	static long f(long m,long n)
	{
		...
		while (tm>0)
		{
			if (tm%2==0 && ontail)
				l++;
			if (tm%2==1) ontail=false;
			tm=tm/2;
		}
		long n1=m+(1<<l);
		if (n1>n) return m;
		else return m&f(n1,n);
	}
...
class Solution209
{
	...
	int sch(int s,int l,int r,int t)
	{
		if (l>r) return -1;
		if (l==r) 
			if (ss(s,r)>=t) return r;
			else return -1;
		if (r==l+1)
		{
			if (ss(s,l)>=t) return l;
			else if (ss(s,r)>=t) return r;
			else return -1;
		}
		int m=(l+r)/2;
		if (ss(s,m)>=t) return sch(s,l,m,t);
	...
	public int minSubArrayLen(int s, int[] nums)
	{
		...
		if (ps[nums.length-1]<s) return 0;
		for (int i=0;i<nums.length;i++)
		{
			if (ss(i,nums.length-1)<s) break;
			int t=sch(i,i,nums.length-1,s);
			if (t!=-1 && t-i+1<minl) minl=t-i+1;
		}
		return minl;
	}
}
...
public class LC201_210 {
	...
	{
		System.out.println(
				new Solution209().minSubArrayLen(
