...
class Solution773
{
	...
	int toInt(int[] x)
	{
		return x[5] + x[4] * 6 + x[3] * 36 + x[2] * 216 + x[1] * 1296 + x[0] * 7776;
	}

	boolean success(int[] x)
	{
		return x[5] == 0 && x[4] == 5 && x[3] == 4 && x[2] == 3 && x[1] == 2 && x[0] == 1;
	}
		...
		for (int i = 0; i < m; i++)
			for (int j = 0; j < n; j++)
				for (int k = 0; k < 4; k++)
				{
					xs = i + di[k][0];
					ys = j + di[k][1];
					if (xs >= 0 && xs < m && ys >= 0 && ys < n && f[xs][ys] > Math.max(f[i][j], g[xs][ys]))
					{
...
class Solution780
{
	public boolean reachingPoints(int sx, int sy, int tx, int ty)
	{
		...
		if (reachingPoints(sx + sy, sy, tx, ty))
			return true;
		if (reachingPoints(sx, sy + sx, tx, ty))
			return true;
		return false;
	}
}
