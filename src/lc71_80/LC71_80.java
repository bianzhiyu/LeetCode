...
class Solution72
{
	public int minDistance(String word1, String word2)
	{
		...
		while (Q.size() > 0)
		{
			...
			for (int j = 0; j < 26; j++)
			{
				nw = new String(new char[]
				{ (char) (j + (int) 'a') }) + w;
				if (word2.compareTo(nw) == 0)
					return s + 1;
				if (!H.contains(nw))
				{
					Q.add(nw);
			...
			for (int i = 0; i < w.length() - 1; i++)
				for (int j = 0; j < 26; j++)
				{
					nw = w.substring(0, i + 1) + new String(new char[]
					{ (char) (j + (int) 'a') }) + w.substring(i + 1, w.length());
					if (word2.compareTo(nw) == 0)
						return s + 1;
					if (!H.contains(nw))
					{
			...
			{
				nw = w.substring(0, i) + w.substring(i + 1, w.length());
				if (word2.compareTo(nw) == 0)
					return s + 1;
				if (!H.contains(nw))
				{
			...
			for (int i = 0; i < w.length(); i++)
			{
				for (int j = 0; j < 26; j++)
				{
					if (j == (int) w.charAt(i) - (int) 'a')
						continue;
					nw = w.substring(0, i) + new String(new char[]
					{ (char) (j + (int) 'a') }) + w.substring(i + 1, w.length());
					if (word2.compareTo(nw) == 0)
						return s + 1;
					if (!H.contains(nw))
		...
		for (int i = 1; i <= len1; i++)
		{
			...
			for (int j = 1; j <= len2; j++)
			{
				if (word1.charAt(i - 1) == word2.charAt(j - 1))
					...
					d[i][j] = 1 + Math.min(d[i - 1][j - 1], // replace
		...
		for (int i = 1; i <= len1; i++)
		{
			...
			for (int j = 1; j <= len2; j++)
			{
				if (word1.charAt(i - 1) == word2.charAt(j - 1))
					...
					e[j] = 1 + Math.min(e[j - 1], Math.min(d[j - 1], d[j]));
...
class Solution74
{
	...
	{
		if (l == r)
			return getVal(m, cn, l) == target;
		if (r == l + 1)
			return getVal(m, cn, l) == target || getVal(m, cn, r) == target;
		int mid = (l + r) / 2;
		if (getVal(m, cn, mid) == target)
			return true;
	...
	public boolean searchMatrix(int[][] matrix, int target)
	{
		if (matrix.length == 0 || matrix[0].length == 0)
			return false;
		return search(matrix, matrix[0].length, 0, matrix[0].length * matrix.length - 1, target);
	}
}
...
class Solution76
{
	...
	void disp(int[] appct)
	{
		for (int i = 0; i < 256; i++)
			if (appct[i] > 0)
				System.out.println((char) i + ": " + appct[i] + "  ");
	}
...
class Solution79
{
	boolean fd(char[][] b, int x, int y, char[] s, int sp)
	{
		...
		if (x < 0 || x >= b.length || y < 0 || y >= b[0].length || b[x][y] != s[sp] || b[x][y] == '$')
			return false;
		...
		char c = b[x][y];
		b[x][y] = '$';
		if (fd(b, x + 1, y, s, sp + 1))
			return true;
		if (fd(b, x, y + 1, s, sp + 1))
			return true;
		if (fd(b, x - 1, y, s, sp + 1))
			return true;
		if (fd(b, x, y - 1, s, sp + 1))
...
public class LC71_80
{
	public static void main(String[] args)
	{
		char[][] b =
		{
				...
				{ 'S', 'F', 'C', 'S' },
				{ 'A', 'D', 'E', 'E' } };
		System.out.println(new Solution79().exist(b, "ABCCED"));
